<script lang="ts">
	...
	let { pages, onSelect, onCreate } = $props<{
		pages: { id: string, title: string, icon: string, active: boolean }[],
...
<nav class="w-72 border-l border-white/5 p-6 flex flex-col gap-6 bg-[#050507] z-20">
	...
	<div class="mt-auto pt-6 border-t border-white/5 space-y-2">
		...
		<button class="w-full p-2 text-[10px] text-slate-600 hover:text-slate-400 text-left transition-colors uppercase tracking-widest">Archive</button>
	</div>
</nav>
